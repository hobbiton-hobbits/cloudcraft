...
const getMessages = async (sender_id, recipient_id = null, group_id = null) => {
  ...
  if (recipient_id) {
    return await pool.query(`SELECT message_text, created FROM messages WHERE sender_id = $1 AND recipient_id = $2 OR WHERE sender_id = $2 AND recipient_id = $1 ORDER BY message_id DESC;`, [sender_id, recipient_id])
  }
  if (group_id) {
    return await pool.query(`SELECT message_text, created FROM messages WHERE sender_id = $1 AND recipient_group_id = $2 OR WHERE sender_id = $2 AND recipient_group_id = $1 ORDER BY message_id DESC;`, [sender_id, group_id])
  }
}
